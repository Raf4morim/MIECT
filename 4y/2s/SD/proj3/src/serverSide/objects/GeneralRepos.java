 ...
 * General Repository.
 * 
 * It is responsible to keep the visible internal state of the problem and
 * provide
...
public class GeneralRepos implements GeneralReposInterface {
    ...
    public synchronized void setCoachState(int id, int state) throws RemoteException {
        coachState[id] = state;
        reportStatus();
    }
    ...
    public synchronized void setContestantState(int id, int state) throws RemoteException  {
    ...
    public synchronized void setContestantStateAndStrength(int id, int state, int strength, int teamID) throws RemoteException {
        contestantState[id] = state;
    ...
    public synchronized void setRefereeStateAndNGames(int state, int nGames) throws RemoteException {
        refereeState = state;
        this.nGames = nGames;
        reportStatus();
    }
    ...
    public synchronized void setRefereeStateAndMatchWinner(int state, int matchWinner, int score0, int score1) throws RemoteException {
        refereeState = state;
    ...
    public synchronized void setPosRope(int posRope) throws RemoteException {
    ...
    public synchronized void setNTrials(int nTrials) throws RemoteException {
     ...
     */
    public synchronized void setGameWinner(int gameWinner) throws RemoteException {
     ...
     *
     * The referee is starting a match, the coaches are waiting for the referee
     * command
    ...
    private void reportStatus() {
        ...
        if (RefereeState.END_OF_A_GAME == refereeState) {
            if (Math.abs(posRope) >= 4) {
                log.writelnString("Game " + nGames + " was won by team " + gameWinner + " by knock out in " + nTrials
                        + " trials.");
            } else if (posRope == 0) {
        ...
        if (RefereeState.END_OF_THE_MATCH == refereeState) {
            if (matchWinner == 2) {
                ...
                log.writelnString("Match was won by team " + matchWinner + " (" + score0 + "-" + score1 + ").");
            }
        }
