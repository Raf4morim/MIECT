...
esp_err_t tc74_init(i2c_master_bus_handle_t busHandle, i2c_master_dev_handle_t* pSensorHandle, uint8_t sensorAddr, int sdaPin, int sclPin, uint32_t clkSpeedHz)
{
    i2c_config_t conf = {
    ...
    i2c_param_config(busHandle, &conf);

    esp_err_t ret = i2c_driver_install(busHandle, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0);
    if (ret != ESP_OK) {
        return ret;
    }

    *pSensorHandle = i2c_master_init(busHandle, &conf);
